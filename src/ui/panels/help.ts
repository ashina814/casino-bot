    ...
    .addFields(
      ...
      },
      {
        ...
        value: [
          ...
          "🎴 `/勝負 丁半` — 丁半（胴が振り、丁/半に分かれて張る多人数戦）",
          "🎲 `/勝負 チンチロ` — チンチロ 1対1（賭けて勝者総取り）",
          "⚔️ `/勝負 サシ` — 1対1の私的決闘（エスクロー・自己申告）",
          "📋 `/勝負 板 立てる` — 何でも賭けられる公開市場（議題立て→賭ける→精算）",
        ].join("\n"),
      },
      {
        name: "🎴 勝負の卓（VC）",
        value: [
          "`/勝負 サシ` `/勝負 板` の中の **「卓を立てる」ボタン** で、その勝負専用のVCが立つ。",
          "勝負が終わったら **続行/やめる** を選べる。何もしないと 5 分で片付くよ。",
        ].join("\n"),
      },
      {
        name: "📈 投資系",
  ...
  return baseEmbed("💼 ヘルプ — 経済の仕組み", COLORS.GOLD)
    .addFields(
      ...
      },
      {
        name: "🥇 星位と賭け上限",
        value: [
          "プレイすると **星の力 (EXP)** が貯まり、レベルが上がる。レベルで星位（漂着者→星拾い→星渡り→星詠み→北極星）が昇格し、**1回の賭け上限**が解放される。",
          "現在の星位・賭け上限は `/通行証` で確認。",
        ].join("\n"),
      },
      {
  ...
  return baseEmbed("🔤 ヘルプ — 用語集", COLORS.GOLD)
    .addFields(
      ...
      },
      {
        ...
        value: [
          "**星の力 (EXP)**: プレイで貯まる経験値",
          "**星位 (tier)**: 漂着者→星拾い→星渡り→星詠み→北極星。賭け上限が変わる",
