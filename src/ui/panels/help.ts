    ...
    .addFields(
      {
        name: "🏠 単独プレイ（`/遊ぶ` 経由）",
        value: [
          "🎰 `/遊ぶ 巻物` — 百鬼夜行巻物（スロット、ワイルド/スキャッター/JP有り）",
          "🎴 `/遊ぶ 丁半` — 丁半博打（ハイ/ロー）",
          "🃏 `/遊ぶ 札遊び` — 花札勝負（ブラックジャック）",
          "📈 `/遊ぶ 暴落` — 龍脈昇り（最低降車1.5x、降りるタイミング）",
      ...
      },
      {
        name: "👥 みんなで",
        value: [
          "🎡 `/遊ぶ 輪盤` — 百鬼輪盤（赤黒奇偶緑、複数人参加可）",
          "🏇 `/競馬 start` — 神馬競走（単勝・複勝のパリミューチュエル）",
        ].join("\n"),
      },
      {
        name: "📈 投資系",
        value: [
          "`/龍脈` — 龍脈相場（5銘柄、3時間ごと値動き、スパークラインで履歴可視化）",
        ].join("\n"),
      },
    );
}
