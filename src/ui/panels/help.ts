    ...
    .addFields(
      {
        name: "🏠 単独プレイ（`/遊ぶ` 経由）",
        value: [
          "🎰 `/遊ぶ 巻物` — 星辰の巻（スロット、ワイルド/スキャッター/JP有り）",
          "🎴 `/遊ぶ 丁半` — 丁半博打（ハイ/ロー）",
          "🃏 `/遊ぶ 札遊び` — 星札勝負（ブラックジャック）",
          "📈 `/遊ぶ 暴落` — 星昇り（最低降車1.5x、降りるタイミング）",
          "🎲 `/遊ぶ 賽` — チンチロ（対座敷童 タイマン、役比べ）",
        ].join("\n"),
      },
      {
        name: "👥 みんなで",
        value: [
          "🎡 `/遊ぶ 輪盤` — 運命の星盤（赤黒奇偶緑、複数人参加可）",
          "🏇 `/競馬 start` — 天馬競走（単勝・複勝のパリミューチュエル）",
        ].join("\n"),
      },
      {
        name: "📈 投資系",
        value: [
          "`/星脈` — 星脈相場（5銘柄、3時間ごと値動き、スパークラインで履歴可視化）",
        ].join("\n"),
      },
    );
}
  ...
  return baseEmbed("💼 ヘルプ — 経済の仕組み", COLORS.GOLD)
    .addFields(
      ...
      },
      {
        name: "⚖️ 福の重み（累進奉納）",
        value: [
          "所持金が多くなるほど、**勝利金の一部が自動で奉納**される（5%〜30%）。座敷童の覚醒が進むと軽減される。",
      ...
      },
      {
        name: "🏆 JPプール（スロット）",
        value: [
          "スロットの賭金の **1%が累積**。座敷童3つ揃いで **プールの半分を獲得**。残り半分は次回シードに残留。",
          "現在のプール額はスロット画面で常時表示。",
        ].join("\n"),
      },
    );
  ...
  return baseEmbed("✦ ヘルプ — アステルとの星約", COLORS.GOLD)
    .addFields(
      ...
      },
      {
        name: "🎭 セリフモード",
        value: [
          "覚醒段階で **セリフモード**（通常 / ツンデレ / ヤミ）が解放される。",
          "`/座敷童 mode` で切替。今のところ **セリフ味の変化のみ**（ゲーム効果はない）。",
        ].join("\n"),
      },
      {
        name: "🪷 五行属性（Lv3〜）",
        value: [
          ...
          "現状の効果: 主に **株（星脈相場）** で相性ボーナス。Lv6で「神柱」名が判明。",
        ].join("\n"),
      },
      {
        name: "🛡️ 身代わりの加護",
  ...
  return baseEmbed("✨ ヘルプ — 隠し・レアの世界", COLORS.GOLD)
    .addFields(
      ...
      },
      {
        name: "✨ レアイベント（覚醒で起こる）",
        value: [
          "覚醒が進むと、稀に座敷童の気まぐれが起こる：",
          "・**座敷童の夢** — 福分けで稀に、ボーナスエテル",
          "・**福の奔流** — 勝利時に稀に、配当が膨らむ",
          "・**花散らしの守り** — 敗北時に稀に、賭金返還",
          "・**神隠し** — 高位覚醒の極稀な現象",
          "*（発動率は伏せる。覚醒を上げて自分で出会え）*",
        ].join("\n"),
      },
      {
        name: "🔍 隠しコマンド・遊び",
